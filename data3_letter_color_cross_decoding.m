%% data3 letter-color cross-family sequential LDA decoding
% Reads first-memory-locked cda/alpha data saved by data3_cda_alpha.m.
% For each set-size comparison, train the load decoder on one memory family
% (letter or color) and test it on the other family. Decoding is still run
% separately within attended-left and attended-right trials, then averaged.
clear; clc;
...
modelsOverride = strtrim(char(getenv('DATA3_LETTER_COLOR_MODELS')));
if ~isempty(modelsOverride)
    modelNames = regexp(modelsOverride, '[,;\s]+', 'split');
    modelNames = modelNames(~cellfun('isempty', modelNames));
end
...
comparisons(1).name = sprintf('letter_color_cross_setsize1_vs6_%s', analysisModeName);
comparisons(1).outputdir = fullfile(dataDir, ...
    sprintf('decoding_LDA_letter_color_cross_setsize1_vs6_segments_%s', analysisModeName));
...
comparisons(2).name = sprintf('letter_color_cross_setsize3_vs6_%s', analysisModeName);
comparisons(2).outputdir = fullfile(dataDir, ...
    sprintf('decoding_LDA_letter_color_cross_setsize3_vs6_segments_%s', analysisModeName));
...
directions = struct( ...
    'name', {'letter_to_color', 'color_to_letter'}, ...
    'trainFamily', {'letter', 'color'}, ...
    'testFamily', {'color', 'letter'}, ...
    'trainMemoryType', {1, 0}, ...
    'testMemoryType', {0, 1});

%% LDA config follows data3_setsize1_vs6_LDA_decoding.m, with explicit train/test families
cfg = struct();
cfg.superTrial = 1;
cfg.nIter = 50;
...
cfg.balanceNPerCell = [];
cfg.useAUC = true;
cfg.saveWeights = false;
cfg.randomSeed = [];
cfg.verbose = false;
cfg.minTrialsPerClass = 4;

nIterOverride = str2double(strtrim(char(getenv('DATA3_LETTER_COLOR_NITER'))));
if ~isnan(nIterOverride) && nIterOverride > 0
    cfg.nIter = nIterOverride;
end

files = dir(fullfile(datadir, 'sub*.mat'));
files = data3_filter_subject_mat_files(files, data3_subject_filter());
if isempty(files)
    error('No sub*.mat files found in %s. Run data3_cda_alpha first.', datadir);
end
...
for sf = 1:numel(files)
    ...
    fprintf('data3 letter-color cross LDA [%s]: %s\n', analysisModeName, file);
    ...
    if ~isfolder(fullfile(behaviorDir, 'Beh_data'))
        error('Cannot find data3 behavior folder: %s', fullfile(behaviorDir, 'Beh_data'));
    end
    ...
    for ci = 1:numel(comparisons)
        ...
        for mi = 1:numel(modelNames)
            ...
            cfgModel = cfg;
            cfgModel.doPCA = strcmpi(modelName, 'PCA');
            ...
            Decode.analysis = 'letter_color_cross_family_sequential_load_decoding';
            ...
            Decode.shuffle = struct('enabled', false, ...
                'reason', 'Disabled for this cross-family decoding; compare against theoretical chance accuracy.');
            Decode.crossFamily = struct('directions', directions, ...
                'memoryTypeCoding', 'behavior MemoryType: 0=color, 1=letter');
            ...
            for di = 1:numel(directions)
                directionName = directions(di).name;
                trainMemoryType = directions(di).trainMemoryType;
                testMemoryType = directions(di).testMemoryType;
                sideResults = struct();
                ...
                for si = 1:numel(sideNames)
                    ...
                    nTrialsByPair = nan(nLowSeg, nHighSeg, 4);
                    ...
                    for li = 1:nLowSeg
                        ...
                        for hi = 1:nHighSeg
                            ...
                            lowTrainIdx = lowBehavior.MemoryType == trainMemoryType;
                            highTrainIdx = highBehavior.MemoryType == trainMemoryType;
                            lowTestIdx = lowBehavior.MemoryType == testMemoryType;
                            highTestIdx = highBehavior.MemoryType == testMemoryType;

                            nTrialsByPair(li, hi, :) = [sum(lowTrainIdx), sum(highTrainIdx), ...
                                sum(lowTestIdx), sum(highTestIdx)];
                            if min(nTrialsByPair(li, hi, :)) < cfgModel.minTrialsPerClass
                                error(['Subject %d %s %s %s has too few trials for cross-family decoding ', ...
                                    '(lowTrain=%d, highTrain=%d, lowTest=%d, highTest=%d).'], ...
                                    subject, modelName, directionName, sideName, ...
                                    nTrialsByPair(li, hi, 1), nTrialsByPair(li, hi, 2), ...
                                    nTrialsByPair(li, hi, 3), nTrialsByPair(li, hi, 4));
                            end

                            trainData = cat(1, lowWin(lowTrainIdx,:,:), highSeg(highTrainIdx,:,:));
                            testData = cat(1, lowWin(lowTestIdx,:,:), highSeg(highTestIdx,:,:));
                            trainLabels = [seq.labelLow * ones(sum(lowTrainIdx), 1); ...
                                           seq.labelHigh * ones(sum(highTrainIdx), 1)];
                            testLabels = [seq.labelLow * ones(sum(lowTestIdx), 1); ...
                                          seq.labelHigh * ones(sum(highTestIdx), 1)];

                            trainData = permute(trainData, [2 3 1]);
                            testData = permute(testData, [2 3 1]);

                            segResult = LDA_crossSide_singleSubj( ...
                                trainData, trainLabels, testData, testLabels, relativeTime, cfgModel);
                            ...
                            AccTrainByPair(:,li,hi) = segResult.AccTrain(:); %#ok<SAGROW>
                    ...
                    resSide.nTrialsByPair = nTrialsByPair;
                ...
                for fi = 1:numel(fieldsNow)
                    ...
                    if isfield(sideResults.R, f) && isnumeric(sideResults.L.(f)) && ...
                        ...
                        DirectionDecode.(f) = mean(cat(dim, sideResults.L.(f), sideResults.R.(f)), dim, 'omitnan');
                    end
                end
                if isfield(DirectionDecode, 'cfg')
                    DirectionDecode.cfg.withinSideAverage = true;
                end

                DirectionDecode.name = directionName;
                DirectionDecode.trainFamily = directions(di).trainFamily;
                DirectionDecode.testFamily = directions(di).testFamily;
                DirectionDecode.trainMemoryType = trainMemoryType;
                DirectionDecode.testMemoryType = testMemoryType;
                DirectionDecode.side = struct('Left', sideResults.L, 'Right', sideResults.R);
                DirectionDecode.summary = struct();
                DirectionDecode.summary.meanAcc = mean(DirectionDecode.Acc(:), 'omitnan');
                if isfield(DirectionDecode, 'AUC')
                    DirectionDecode.summary.meanAUC = mean(DirectionDecode.AUC(:), 'omitnan');
                else
                    DirectionDecode.summary.meanAUC = NaN;
                end

                Decode.direction(di) = DirectionDecode;

                row = table(subject, {comparisons(ci).name}, {modelName}, {directionName}, ...
                    {directions(di).trainFamily}, {directions(di).testFamily}, ...
                    DirectionDecode.summary.meanAcc, DirectionDecode.summary.meanAUC, ...
                    seq.chanceAccuracy, ...
                    'VariableNames', {'Subject', 'Comparison', 'Model', 'Direction', ...
                    'TrainFamily', 'TestFamily', 'MeanAcc', 'MeanAUC', 'ChanceAccuracy'});
...
for ci = 1:numel(comparisons)
    SummaryThisComparison = SubjectSummary(strcmp(SubjectSummary.Comparison, comparisons(ci).name), :);
    writetable(SummaryThisComparison, fullfile(comparisons(ci).outputdir, ...
        sprintf('%s_subject_summary.csv', comparisons(ci).name)));
    runCfg = struct('cfg', cfg, 'directions', directions, 'comparison', comparisons(ci));
    save(fullfile(comparisons(ci).outputdir, 'run_cfg.mat'), 'runCfg');
end

fprintf('data3 letter-color cross LDA results saved under:\n%s\n', dataDir);
