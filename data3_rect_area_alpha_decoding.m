%% data3 rectangular bounding-box area matched sequential Alpha/GlobalAlpha/CDA LDA decoding
% This script reruns data3 set-size-3 vs set-size-6 sequential decoding after
% matching trials on a coarse rectangular bounding-box area proxy.
%
% It does NOT modify data3_spatial_matched_alpha_decoding.m. It reads the
% first-memory-locked cda/alpha files saved by data3_cda_alpha.m and writes a
% new output folder.
%
% Main control:
%   cfg.controlMode = 'rectBBoxAreaBin'
%   The script computes cumulative rectangular bounding-box area up to each
%   serial position, then bins low/high trials by pooled median area within
%   each side x low-position x high-position pair. LDA_function_singleSubj
%   then balances label x area-bin cells.
%
% BBox area modes:
%   'fixationAnchored' includes the fixation point [0,0] in the bounding box.
%       This makes set-size-1 area depend on location/eccentricity and is more
%       useful if the goal is a coarse spatial-attention extent proxy.
%   'stimOnly' uses only stimulus rectangles. In this mode set-size-1 area is
%       constant, so it cannot by itself use set-size-1 location differences.
...
cfg.lowSetSize = 3;
cfg.highSetSize = 6;
cfg.modelNames = {'GlobalAlpha'};%{, 'Alpha', 'CDA'}; % Alpha/global alpha targets; CDA is a positive-control comparison

% Spatial control mode for this standalone script.
%   'rectBBoxAreaBin' = match a coarse cumulative rectangular bounding-box area.
cfg.controlMode = 'rectBBoxAreaBin';
cfg.cumulativeMetric = 'CumBBoxArea';
cfg.nSpatialBins = 2;                     % median split by default
cfg.minFactorCellN = 2;
cfg.minBalancedTrialsPerClass = 10;
cfg.rectHalfSizePx = 30;                  % fixed square proxy, side length = 60 px
cfg.rectHalfWidthPx = cfg.rectHalfSizePx;
cfg.rectHalfHeightPx = cfg.rectHalfSizePx;
cfg.bboxAreaMode = 'fixationAnchored';    % 'fixationAnchored' or 'stimOnly'
cfg.outputTag = strtrim(char(getenv('DATA3_RECTBBOX_OUTPUT_TAG')));

areaModeOverride = strtrim(char(getenv('DATA3_RECTBBOX_AREA_MODE')));
if ~isempty(areaModeOverride)
    cfg.bboxAreaMode = areaModeOverride;
end

halfSizeOverride = str2double(strtrim(char(getenv('DATA3_RECTBBOX_HALF_SIZE'))));
if ~isnan(halfSizeOverride) && halfSizeOverride > 0
    cfg.rectHalfSizePx = halfSizeOverride;
    cfg.rectHalfWidthPx = halfSizeOverride;
    cfg.rectHalfHeightPx = halfSizeOverride;
end

modelsOverride = strtrim(char(getenv('DATA3_RECTBBOX_MODELS')));
...
nPermOverride = str2double(strtrim(char(getenv('DATA3_RECTBBOX_NPERM'))));
if ~isnan(nPermOverride) && nPermOverride > 0
    cfg.nPerm2D = nPermOverride;
end

makeStatsOverride = strtrim(char(getenv('DATA3_RECTBBOX_MAKE_STATS')));
if ~isempty(makeStatsOverride)
    cfg.makeGroupStats = ~ismember(lower(makeStatsOverride), {'0', 'false', 'no'});
end

makeFiguresOverride = strtrim(char(getenv('DATA3_RECTBBOX_MAKE_FIGURES')));
...
comparisonName = sprintf('setsize%d_vs%d_%s_rectBBoxAreaMatched_%s', ...
    cfg.lowSetSize, cfg.highSetSize, cfg.analysisModeName, cfg.controlMode);
outputdir = fullfile(dataDir, sprintf('decoding_LDA_setsize%d_vs%d_segments_%s_rectBBoxAreaMatched_%s', ...
    cfg.lowSetSize, cfg.highSetSize, cfg.analysisModeName, cfg.controlMode));
...
nIterOverride = str2double(strtrim(char(getenv('DATA3_RECTBBOX_NITER'))));
...
for sf = 1:numel(files)
    ...
    fprintf('data3 rect-bbox-area LDA [%s]: %s\n', cfg.controlMode, file);
    ...
    for mi = 1:numel(cfg.modelNames)
        ...
        switch lower(modelName)
            case {'alpha', 'cda', 'globalalpha', 'globalalphamean'}
                sourceData = savedData.alpha;
                featureMode = 'lateralized';
                featureConstruction = 'Constructed as alpha contra-minus-ipsi within attended side.';

                if strcmpi(modelName, 'CDA')
                    sourceData = savedData.cda;
                    modelData.channelLabels = savedData.cda.relPairLabels;
                    featureConstruction = 'Constructed as CDA contra-minus-ipsi within attended side.';
                ...
                elseif strcmpi(modelName, 'GlobalAlpha')
                    ...
                    featureConstruction = 'Constructed as absolute posterior alpha [left channels, right channels].';
                else
                    featureMode = 'globalmean';
                    modelData.channelLabels = {'posteriorAlphaMean'};
                    featureConstruction = 'Constructed as the trial-wise mean over absolute posterior alpha channels.';
                end
            otherwise
                error('This script supports Alpha, GlobalAlpha, GlobalAlphaMean, and CDA. Requested: %s.', modelName);
        end
        ...
        for si = 1:numel(sideNames)
            ...
            for li = 1:numel(loadNames)
                ...
                switch lower(featureMode)
                    ...
                    case 'global'
                        modelData.(sideName).(loadNames{li}) = cat(2, leftX, rightX);
                    case 'globalmean'
                        modelData.(sideName).(loadNames{li}) = mean(cat(2, leftX, rightX), 2, 'omitnan');
                    otherwise
            ...
            for li = 1:nLowSeg
                ...
                for hi = 1:nHighSeg
                    ...
                    switch lower(cfg.controlMode)
                        case 'rectbboxareabin'
                            % Coarse area matching: pooled median/quantile bins within this
                            % side x low-position x high-position pair. The later LDA call
                            % balances label x area-bin cells through cfgPair.balanceFactors.
                            lowMetric = lowMetricBefore;
                            highMetric = highMetricBefore;
                            metricAll = [lowMetric(:); highMetric(:)];
                            metricAll = metricAll(~isnan(metricAll));

                            if isempty(metricAll)
                                skipReason = 'missingBBoxArea';
                                lowFactor = nan(numel(lowMetric), 1);
                                highFactor = nan(numel(highMetric), 1);
                            elseif max(metricAll) - min(metricAll) < 1e-9
                                lowFactor = ones(numel(lowMetric), 1);
                                highFactor = ones(numel(highMetric), 1);
                                lowFactor(isnan(lowMetric)) = NaN;
                                highFactor(isnan(highMetric)) = NaN;
                            else
                                q = linspace(0, 1, cfg.nSpatialBins + 1);
                                edges = quantile(metricAll, q);
                    ...
                    M.BBoxAreaMode = {cfg.bboxAreaMode};
                    M.RectHalfWidthPx = cfg.rectHalfWidthPx;
                    M.RectHalfHeightPx = cfg.rectHalfHeightPx;
                    ...
                    M.MeanLowBBoxAreaBefore = mean(lowMetricBefore, 'omitnan');
                    M.MeanHighBBoxAreaBefore = mean(highMetricBefore, 'omitnan');
                    M.MeanDiffHighMinusLowBBoxAreaBefore = M.MeanHighBBoxAreaBefore - M.MeanLowBBoxAreaBefore;
                    M.MeanLowBBoxAreaAfter = meanLowMetricAfter;
                    M.MeanHighBBoxAreaAfter = meanHighMetricAfter;
                    M.MeanDiffHighMinusLowBBoxAreaAfter = M.MeanHighBBoxAreaAfter - M.MeanLowBBoxAreaAfter;
        ...
        Decode.spatialControl.rectHalfWidthPx = cfg.rectHalfWidthPx;
        Decode.spatialControl.rectHalfHeightPx = cfg.rectHalfHeightPx;
        Decode.spatialControl.bboxAreaMode = cfg.bboxAreaMode;
        ...
        Decode.spatialControl.description = ['Cumulative rectangular bounding-box area is computed before LDA within each side x low-segment x high-segment pair. ' ...
            'The decoder then balances label x area-bin cells using LDA_function_singleSubj.'];
        ...
        Decode.featureConstruction = featureConstruction;
...
fprintf('\nRectangular BBox area-matched sequential LDA finished.\nOutput folder:\n%s\n', outputdir);
