...
pchans = ismember(erp.allChans,{'O1','O2','OL','OR','PO3','PO4','POz','T5','T6'}); 
fchans = ismember(erp.allChans,{'F3','F4','Fz'}); 

cls.acc_task_allchans =  NaN(nSubs,nBins); 
cls.acc_task_allchans_shuffle =  NaN(nSubs,nBins); 
cls.acc_task_pchans =  NaN(nSubs,nBins); 
cls.acc_task_fchans =  NaN(nSubs,nBins); 

cls.weights_task_allchans =  NaN(nSubs,nBins,20); 
cls.weights_task_allchans_shuffle =  NaN(nSubs,nBins,20); 
% cls.weights_task_pchans =  NaN(nSubs,nBins,sum(pchans)); 
% cls.weights_task_fchans =  NaN(nSubs,nBins,sum(fchans)); 
...
for s = 1:nSubs
    ...
    for b = 1:nBins
        ...
        % Save accuracy!
        cls.acc_task_allchans(s,b) = 1 - classLoss;
        cls.weights_task_allchans(s,b,:) = svm.Mu; % weights? 
        ...
        classLoss = kfoldLoss(cval);
        % Save accuracy!
        cls.acc_task_allchans_shuffle(s,b) = 1 - classLoss;
        cls.weights_task_allchans_shuffle(s,b,:) = svm.Mu; % weights? 
        ...
        classLoss = kfoldLoss(cval);
        % Save accuracy!
        cls.acc_task_pchans(s,b) = 1 - classLoss;
%         cls.weights_task_pchans(s,b,:) = svm.Mu; % weights? 
        ...
        classLoss = kfoldLoss(cval);
        % Save accuracy!
        cls.acc_task_fchans(s,b) = 1 - classLoss;
%         cls.weights_task_fchans(s,b,:) = svm.Mu; % weights? 
...
save([pwd,'/CDA_classification/task_classify.mat'],'cls','subjects')
