...
cls.chanLabels = erp.allChans(1:20);
 
nSubs = length(subjects);
nBins = length(tBins) - 1;
nChans = length(cls.chanLabels);
    ...
    for ch = 1:nChans
        ...
        for b = 1:nBins
            for it = 1:nIter
                ...
                allCh = 1:nChans;
                tDat = squeeze(nanmean(tempDat(:,~ismember(allCh,ch),ismember(tPts,tBins(b):tBins(b+1))),3));
        ...
        cls.acc_load_allchans(s,ch,:) = nanmean(cls_acc_load_allchans,1); % Average across iterations
        cls.acc_load_allchans_shuffle(s,ch,:) = nanmean(cls_acc_load_allchans_shuffle,1);
        
    end % end eletrode loop
...
save([pwd,'/CDA_classification/load_classify_diagLinear_iterations_leave1electrode.mat'],'cls')
