...
for s = 1:nSubs
    ...
    tempDat = [erp.trial.R_C2; erp.trial.R_C6; erp.trial.R_S2; erp.trial.R_S6];
    
    labels = [ones(size(erp.trial.R_C2,1),1)*2; ones(size(erp.trial.R_C6,1),1)*6; ...
    ...
    for b = 1:nBins
        for it = 1:nIter
            ...
            ss2Ind = find(labels==2); ss2Ind = ShuffleKA(ss2Ind); ss2Ind = ss2Ind(1:minT);
            ss6Ind = find(labels==6); ss6Ind = ShuffleKA(ss6Ind); ss6Ind = ss6Ind(1:minT);
...
save([pwd,'/CDA_classification/load_classify_diagLinear_trainRtestR.mat'],'cls')
