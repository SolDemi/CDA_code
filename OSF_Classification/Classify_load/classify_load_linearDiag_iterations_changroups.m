...
for s = 1:nSubs
    ...
    cls_acc_load_pchans = NaN(nIter,nBins);
    cls_acc_load_fchans = NaN(nIter,nBins);
    cls_acc_load_cchans = NaN(nIter,nBins);
    ...
    for b = 1:nBins
        for it = 1:nIter
            ...
            % Save accuracy!
            cls_acc_load_pchans(it,b) = 1 - errRate;
            ...
            % Save accuracy!
            cls_acc_load_fchans(it,b) = 1 - errRate;
            ...
            % Save accuracy!
            cls_acc_load_cchans(it,b) = 1 - errRate;
    ...
    cls.acc_load_pchans(s,:) = nanmean(cls_acc_load_pchans,1);
    cls.acc_load_fchans(s,:) = nanmean(cls_acc_load_fchans,1);
    cls.acc_load_cchans(s,:) = nanmean(cls_acc_load_cchans,1);

    fprintf(sprintf('\n Subject %d out of %d finished \n',s,nSubs));
end % end subject loop

save([pwd,'/CDA_classification/load_classify_diagLinear_iterations_chanGroups.mat'],'cls')
