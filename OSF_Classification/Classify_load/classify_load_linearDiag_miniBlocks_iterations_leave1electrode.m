...
% information!
%
% See if we can classify with just 1 single electrode... and which are most
% informative in a topo map, if so.
...
for bk = 1:length(blockSizes)
    ...
    cls.acc_load_allchans =  NaN(nSubs,nChans,nBins);
    cls.acc_load_allchans_shuffle =  NaN(nSubs,nChans,nBins);
    ...
    for s = 1:nSubs
        ...
        cls_acc_load_allchans = NaN(nIterations,nChans,nBins);
        cls_acc_load_allchans_shuffle = NaN(nIterations,nChans,nBins);
        
        tic
        % Loop through timepoints
        parfor b = 1:nBins % parfor
            for ch = 1:nChans
                for it = 1:nIterations
                    %------------------------------------------------------------------
                    % All electrodes
                    ...
                    allCh = 1:nChans;
                    tDat = squeeze(nanmean(tempDat(:,it,~ismember(allCh,ch),ismember(tPts,tBins(b):tBins(b+1))),4));
                    ...
                    % Save accuracy!
                    cls_acc_load_allchans(it,ch,b) = 1 -  errRate;
...
%                     fprintf('.');
                    ...
                    % Save accuracy!
                    cls_acc_load_allchans_shuffle(it,ch,b) = 1 - errRate;
    ...
    fName = sprintf('/CDA_classification/load_classify_diagLinear_leave1chan_%dit_block%d.mat',cls.iterations,cls.blockSize);
    save([pwd,fName],'cls')
