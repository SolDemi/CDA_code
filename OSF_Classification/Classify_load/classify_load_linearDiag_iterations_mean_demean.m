...
cls.acc_load_demean =  NaN(nSubs,nBins);
cls.acc_load_mean =  NaN(nSubs,nBins);
cls.acc_load_shuffle =  NaN(nSubs,nBins);
...
for s = 1:nSubs
    ...
    cls_acc_load_demean = NaN(nIter,nBins);
    cls_acc_load_mean = NaN(nIter,nBins);
    cls_acc_load_shuffle = NaN(nIter,nBins);
    ...
    for b = 1:nBins
        for it = 1:nIter
            ...
            tDat1 = squeeze(nanmean(tempDat(:,1:20,ismember(tPts,tBins(b):tBins(b+1))),3));
            ...
            % Save accuracy!
            cls_acc_load_demean(it,b) = 1 -  errRate;
            ...
            % Save accuracy!
            cls_acc_load_shuffle(it,b) = 1 - errRate;
            
            %------------------------------------------------------------------
            % Mean only
            %------------------------------------------------------------------           
            % Just the global mean of all electrodes! (1 predictor)
            ...
            % Save accuracy!
            cls_acc_load_mean(it,b) = 1 -  errRate;
    ...
    cls.acc_load_demean(s,:) = nanmean(cls_acc_load_demean,1); % Average across iterations
    cls.acc_load_mean(s,:) = nanmean(cls_acc_load_mean,1);
    cls.acc_load_shuffle(s,:) = nanmean(cls_acc_load_shuffle,1);

    fprintf(sprintf('\n Subject %d out of %d finished \n',s,nSubs));
end % end subject loop

save([pwd,'/CDA_classification/load_classify_diagLinear_iterations_mean_demean.mat'],'cls')
